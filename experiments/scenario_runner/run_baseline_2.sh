            ...
            for target_speed in ${target_speeds[@]}; do
                ...
                if [ ! -f "${PYLOT_HOME}/${file_base}.csv" ]; then
                    ...
                    cd $PYLOT_HOME ; python3 pylot.py --flagfile=configs/scenarios/baseline_2.conf --target_speed=$target_speed --log_file_name=$file_base.log --csv_log_file_name=$file_base.csv \
                        ...
                        --obstacle_detection_model_paths=${PYLOT_HOME}/dependencies/models/obstacle_detection/efficientdet/efficientdet-d0/efficientdet-d0_frozen.pb,${PYLOT_HOME}/dependencies/models/obstacle_detection/yolox/yolox_m.pth \
                        --obstacle_detection_model_names=efficientdet-d0,yolox_m \
                        --yolox_detection_operator  \
                        --simulator_fps=$hz --simulator_control_frequency=$hz \
                        --simulator_camera_frequency=15 --simulator_imu_frequency=10 --simulator_localization_frequency=10 &
